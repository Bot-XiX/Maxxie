const { Client, Collection, Intents, WebhookClient, MessageActionRow, MessageButton } = require("discord.js");
...
module.exports = class BotClient extends Client {
  ...
  async registerGuildInteractions(guild) {
    ...
    try {
      ...
      if (ex.message === "Missing Access") {
        this.logger.debug(`Guild ${guild.name} (${guild.id}) is missing permission to register slash commands`);
        guild.fetchOwner().then((owner) => {
          safeDM(owner.user, {
            content: `I do not have permission to register slash commands in **${guild.name}** (${guild.id})\nPlease reinvite me using the below link`,
            components: [
              new MessageActionRow().addComponents(
                new MessageButton({ style: "LINK", label: "Re-Invite", url: this.getInvite() })
              ),
            ],
          });
        });
